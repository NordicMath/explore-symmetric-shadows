...
\subsection{The Penumbral Ideals}

\begin{definition}
    For a symmetric shadow $\Gamma$, the \emph{penumbral ideal} $P_n$ is a ring-ideal that is the inverse image trough $\Psi_{\Gamma}$ of the penumbral ideal $P_n$ in $\Upsilon_k$. Essentially, it is the ideal of elements that have $\psi^k$-shadow $0$ for $k \le n$.
\end{definition}

\begin{theorem}
    The $P_n$ form a filtration, and the limit of this filtration is the nebula. This hence provides a practical way to compute the nebula.
\end{theorem}
...
%\begin{lemma}
  %If and only if $z \in \Upsilon_k$ has finite dimension $n$, the $\lambda^{n}$-shadow is non-zero and the $\lambda^{l}$-shadow is zero for any $l > n$. 
%\end{lemma}

%\begin{proof}
  %By definition, $z$ having dimension $n$ implies $\lambda^n(z)$ non-zero and $\lambda^{l}(z)$ is zero for any $l > n$. 
%\end{proof}
